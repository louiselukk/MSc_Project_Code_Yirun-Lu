...
def run_regime_matrix_raw(
    run_simulation_fixed_strategy,
    base_params: dict,
    ...
    for mm_s, arb_s, str_s in product(mm_axis, arb_axis, str_axis):
        mkt, agents = run_simulation_fixed_strategy(
            **params,
...
def _rowwise_sum_norm(triple, eps: float = 1e-12):
    m, a, s = map(float, triple)
    tot = m + a + s
    if (m < 0) or (a < 0) or (s < 0) or (tot <= eps):
        shift = max(0.0, -min(m, a, s)) + eps
        m, a, s = m + shift, a + shift, s + shift
        tot = m + a + s
    return [m / tot, a / tot, s / tot]


def run_regime_matrix_rowsum(
    run_simulation_fixed_strategy,
    base_params: dict,
    regime_overrides: dict,
    mm_axis=("passive", "adaptive"),
    ...
    for _, r in df_raw.iterrows():
        ...
        rows.append(
            {
                "MM Strategy": r["MM Strategy"],
                "Arb Strategy": r["Arb Strategy"],
                "STR Strategy": r["STR Strategy"],
                "MM PnL": m,
                "Arb PnL": a,
                "STR PnL": s,
            }
    ...
    df_rowsum = pd.DataFrame(rows)
    if verbose:
        print(df_rowsum.to_string(index=False))
    return df_rowsum


def plot_ternary_rowsum(
    df_rowsum: pd.DataFrame,
    title: str = "Ternary (Row-Sum)",
    fname: str = "results/ternary_rowsum.png",
    dpi: int = 300,
):
    ...
    ax.text(Cx, Cy + 0.02, "STR", fontsize=12, ha="center")
    ...
    for _, row in df_rowsum.iterrows():
        ...
        vals = np.array([m, a, s])
        edge = "black" if np.isclose(vals, vals.max()).sum() == 1 else None

        color = strat_to_color[(row["MM Strategy"], row["Arb Strategy"], row["STR Strategy"])]
        ax.scatter(x, y, marker="o", s=70, color=color, edgecolors=edge, linewidths=1.2 if edge else 0)

    handles = [
        plt.Line2D([0], [0], marker="o", color="w",
                   markerfacecolor=c, markeredgecolor="black", markersize=8,
                   label=f"MM={mm}, Arb={ab}, STR={st}")
